...
adhoc_hub as (
    ...
    from commshub c join exception e on c.commshub_key=e.commshub_key where affected_pm = 'PM 1.1' or affected_pm = 'All Schedule 2.2' and FIRST_VALID_CHSU_RECEIVED_DATE between e.effective_from_dt_tm and e.effective_to_dt_tm and DELETE_FLAG = 'N'
),
bau_hub as (
     select 
     c.COMMSHUB_KEY,o.exception_code as bau_hub_exc_code from commshub c 
     join overrides o on c.commshub_key=o.commshub_key where affected_pm = 'PM 1.1' or affected_pm = 'All Schedule 2.2' and FIRST_VALID_CHSU_RECEIVED_DATE between o.effective_from_dt_tm and o.effective_to_dt_tm
),
joined as (
    SELECT 
    pm_f.COMMSHUB_KEY,
